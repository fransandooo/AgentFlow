...
export function TaskDetail({ task }: { task: any }) {
  ...
  async function createSubtask() {
    ...
    const response = await fetch(`/api/tasks/${task.id}/subtasks`, {
      ...
      body: JSON.stringify({ title: title.trim(), priority: task.priority }),
    });
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-6">
        ...
        <SectionCard
          ...
          title="Subtasks"
          action={
            <button onClick={() => setSubtasksOpen(!subtasksOpen)} className="text-muted">
              {subtasksOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            </button>
          }
        >
          <div className="flex gap-3">
            <Input placeholder="New subtask title" value={title} onChange={(e) => setTitle(e.target.value)} />
            <Button onClick={createSubtask} disabled={!title.trim()}><Plus className="h-4 w-4" /></Button>
          </div>
          {subtasksOpen ? (
            subtasks.length ? (
              subtasks.map((subtask: any) => (
                <div key={subtask.id} className="rounded-[6px] border border-border bg-panelAlt px-5 py-4">
        ...
        <SectionCard icon={<MessageSquareText className="h-5 w-5" />} title="Comments">
          {task.comments?.length ? task.comments.map((comment: any) => (
            <div key={comment.id} className="rounded-[6px] border border-border bg-panelAlt px-5 py-4">
        ...
        <SectionCard icon={<TimerReset className="h-5 w-5" />} title="Activity log">
          {task.activityLogs?.length ? task.activityLogs.map((log: any) => (
            <div key={log.id} className="rounded-[6px] border border-border bg-panelAlt px-5 py-4">
      ...
      <div>
        <Card className="sticky top-28 p-6 sm:p-7">
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-[6px] border border-border bg-panelAlt text-primary">
    ...
    <Card className="p-6 sm:p-7">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-[6px] border border-border bg-panelAlt text-primary">{icon}</div>
...
function MetaRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-start gap-4 rounded-[6px] border border-border bg-panelAlt px-4 py-4">
      <div className="mt-0.5 flex h-9 w-9 items-center justify-center rounded-[6px] border border-border bg-white text-primary">{icon}</div>
      <div>
...
function EmptyState({ text }: { text: string }) {
  return <p className="rounded-[6px] border border-dashed border-border px-5 py-5 text-sm text-muted">{text}</p>;
}
